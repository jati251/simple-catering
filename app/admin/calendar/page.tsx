...
import { ChevronLeft, ChevronRight, Save, Utensils, Loader2, Calendar as CalendarIcon } from 'lucide-react';
import Link from 'next/link';
import { useTranslation, formatIDR, getWIBDate, getWIBDateString } from '@/lib/utils';
...
export default function CalendarConfig() {
  ...
  const [selectedDateStr, setSelectedDateStr] = useState<string>(getWIBDateString());
  ...
  async function fetchData(silent = false) {
    ...
    try {
      const [items, calRecords] = await Promise.all([
        getMenuItems(),
        pb.collection('calendar').getFullList<CalendarItem>({ sort: 'date', expand: 'menu_item', requestKey: null })
      ]);
      setMenuItems(items);
  ...
  const generateCalendarDays = () => {
    ...
    for (let i = 0; i < 35; i++) {
        ...
        const assignments = calendarItems.filter(item => item.date.startsWith(dateStr));
          ...
          <div className="flex items-center justify-between mb-12">
              <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-black text-primary uppercase tracking-widest mt-1">{t.calendar}</p>
                  </div>
              </div>
              <div className="hidden sm:flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 text-[10px] font-black text-primary uppercase">
                 <span className="w-2 h-2 rounded-full bg-primary animate-pulse" /> Live Schedule
              </div>
          </div>
              ...
              <div className="lg:col-span-8 flex flex-col gap-6">
                  <div className="glass p-8 rounded-[3rem] border border-white/5 shadow-2xl relative overflow-hidden">
                      <div className="flex items-center justify-between mb-8">
                          <h2 className="text-2xl font-black italic uppercase tracking-tighter">{t.kitchenSchedule}</h2>
                            ...
                            {calendarDays.map((day, idx) => (
                                ...
                                >
                                    ...
                                    {day.assignments.length > 0 ? (
                                        ...
                                        <div className="text-[8px] font-bold text-muted-foreground/30 opacity-0 group-hover:opacity-100 transition-opacity">{t.empty}</div>
                                    )}
                  ...
                  >
                      <div className="space-y-1">
                          ...
                          <div className="p-3 rounded-xl bg-white/5 border border-white/5 inline-flex items-center gap-2 text-xs font-bold text-primary">
                              ...
                              {new Date(selectedDateStr).toLocaleDateString(t.today === 'Home' ? 'id-ID' : 'en-US', { weekday: 'long', month: 'long', day: 'numeric' })}
                          </div>
                      </div>

                      <div className="space-y-4">
                          <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">{t.currentAssignments}</label>
                          <div className="space-y-2 mb-4">
                            {calendarDays.find(d => d.isSelected)?.assignments.map(a => (
                              <div key={a.id} className="flex items-center justify-between p-3 rounded-xl bg-primary/5 border border-primary/20">
                                <span className="text-[10px] font-bold truncate italic">{a.expand?.menu_item?.name}</span>
                                <button 
                            ...
                            {calendarDays.find(d => d.isSelected)?.assignments.length === 0 && (
                              <p className="text-[10px] text-muted-foreground italic">{t.noMenu}</p>
                            )}
                          </div>

                          <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">{t.menuItems}</label>
                              ...
                              {menuItems.map(item => (
                                  <button
                                      ...
                                      className={`flex items-center gap-3 p-4 rounded-2xl border transition-all text-left group ${
                                          selectedMenuItem === item.id ? 'bg-primary border-primary text-black' : 'bg-white/3 border-white/5 hover:bg-white/5'
                                      }`}
                                  >
                                      ...
                                      <div className="flex-1 pr-2 truncate">
                                          <p className="text-sm font-black italic leading-tight truncate">{item.name}</p>
                                          <p className={`text-[10px] uppercase font-bold tracking-tight ${selectedMenuItem === item.id ? 'text-black/60' : 'text-primary'}`}>{formatIDR(item.price)}</p>
                                      </div>
                                  </button>
                              ))}
                          </div>
                      ...
                      >
                          {saving ? <Loader2 className="animate-spin" /> : <>{t.saveConfig} <Save size={20} /></>}
                      </button>
                      
                      <p className="text-[10px] text-muted-foreground italic text-center leading-relaxed">
                          Data older than today is automatically deleted <br/> by the D'MBG System (WIB).
                      </p>
                  </motion.div>
              </div>
          </div>
