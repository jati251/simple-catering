...
import { Calendar, UtensilsCrossed, LogOut, ChevronRight, PieChart } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12 space-y-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-white/5 pb-8">
        <div>
            <h1 className="text-4xl font-black italic tracking-tighter">{t.adminDashboard}</h1>
            <p className="text-muted-foreground font-medium mt-1">{t.dashboardSub}</p>
        </div>
        <button 
        ...
        <Link href="/admin/menu" className="group">
            ...
            >
                <div className="space-y-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold italic tracking-tight">{t.menuItems}</h2>
                        <p className="text-muted-foreground text-sm mt-2 leading-relaxed">{t.addFood} {stats.menuItems} {t.totalItems}.</p>
                    </div>
                </div>
        ...
        <Link href="/admin/calendar" className="group">
            ...
            >
                <div className="space-y-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold italic tracking-tight">{t.calendar}</h2>
                        <p className="text-muted-foreground text-sm mt-2 leading-relaxed">{t.setSchedule}</p>
                    </div>
                </div>
                <div className="pt-6 flex items-center gap-2 text-primary font-bold text-sm">
      ...
      <div className="flex flex-wrap gap-4 pt-10">
          <div className="px-6 py-3 rounded-2xl bg-white/5 border border-white/5 flex items-center gap-3">
              ...
              <span className="text-sm font-bold text-muted-foreground">{t.systemActive}</span>
          </div>
      </div>
    </div>
  );
