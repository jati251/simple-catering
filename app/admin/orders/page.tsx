...
export default function OrdersManagement() {
  ...
  const filteredOrders = orders.filter(
    (o) =>
      ...
      o.expand?.calendar_id?.expand?.menu_item?.name
        .toLowerCase()
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-6 md:py-12 pb-32">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 md:mb-12">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl md:text-4xl font-black italic tracking-tighter leading-none text-white">
                {t.orders}
              </h1>
            ...
            <table className="w-full text-left min-w-[900px]">
              <thead>
                <tr className="bg-white/5 border-b border-white/5">
                  <th className="px-6 md:px-8 py-4 md:py-6 text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                    {t.buyer}
                  </th>
                  <th className="px-6 md:px-8 py-4 md:py-6 text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                    {t.dateMenu}
                  </th>
                  <th className="px-6 md:px-8 py-4 md:py-6 text-[10px] font-black uppercase tracking-widest text-muted-foreground text-center">
                    {t.paymentProof}
                  </th>
                  <th className="px-6 md:px-8 py-4 md:py-6 text-[10px] font-black uppercase tracking-widest text-muted-foreground text-right tracking-widest">
                    {t.amount}
                  </th>
                  <th className="px-6 md:px-8 py-4 md:py-6 text-[10px] font-black uppercase tracking-widest text-muted-foreground text-right">
                    {t.remove}
                  </th>
                </tr>
              </thead>
                    ...
                    >
                      ...
                      <td className="px-6 md:px-8 py-4 md:py-6 text-center">
                        {order.payment_proof ? (
                          ...
                          <span className="text-[9px] md:text-[10px] font-black text-red-500/50 uppercase italic">
                            {t.missing}
                          </span>
                        )}
                      </td>
                      <td className="px-6 md:px-8 py-4 md:py-6 text-right">
                        <span className="font-black italic text-base md:text-lg text-white">
                          {formatIDR(
                            order.expand?.calendar_id?.expand?.menu_item
                              ?.price || 0,
                          )}
                        </span>
                      </td>
